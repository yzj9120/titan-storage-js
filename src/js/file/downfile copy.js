...
class DownloadScheduler {
    ...
    // 获取下载进度百分比
    getProgress() {
        return Math.floor((this.downloadedSize / this.totalSize) * 100) ?? 0; // 计算下载百分比
    }
...
class DownFile {
    ...
    async downloadChunk(url, start, end, retries = 0) {
        try {
            ...
            if (retries < this.maxRetries) {
                ...
                return { blob: null, size: 0, error: `下载分片失败 ${start}-${end}: ${error.message}` }; // 返回文件的 Blob 和大小
            }
        }
    }
    ...
    async downloadFile(urls, traceId, assetCid, fileName, fileSize) {
        ...
        console.log(`123: ${chunkSize}`); // 输出重试的日志
